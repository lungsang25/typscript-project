      ...
      <div className="max-w-4xl mx-auto px-6">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-bold text-white mb-6">
            Work <span className="bg-gradient-to-r from-[#40E0D0] to-[#40E0D0] bg-clip-text text-transparent">Experience</span>
          </h2>
        ...
        <div className="relative">
          {/* Timeline line */}
          <div className="absolute left-8 top-0 bottom-0 w-0.5 bg-gradient-to-b from-[#40E0D0] to-[#40E0D0]"></div>
          
          <div className="space-y-12">
            {experiences.map((exp, index) => (
              <div key={exp.title} className="relative flex items-start space-x-8">
                {/* Timeline dot */}
                <div className="flex-shrink-0 w-16 h-16 bg-[#40E0D0]/20 rounded-full flex items-center justify-center">
                  <Briefcase className="text-[#40E0D0]" size={24} />
                </div>
                
                {/* Content */}
                <div className="flex-1 bg-slate-700/50 backdrop-blur-sm rounded-2xl p-8 border border-slate-600/50 hover:border-[#40E0D0]/50 transition-all duration-300">
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
                    <div>
                      <h3 className="text-xl font-bold text-white mb-1">{exp.title}</h3>
                      <p className="text-[#40E0D0] font-semibold">{exp.company}</p>
                    </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {exp.technologies.map((tech) => (
                      <span 
                        key={tech}
                        className="px-3 py-1 bg-[#40E0D0]/10 text-[#40E0D0] rounded-full text-xs font-semibold"
                      >
                        {tech}
                      </span>
                    ))}
